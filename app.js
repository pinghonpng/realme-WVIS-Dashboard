...
function render(){ filterData(); const rows=state.filteredSales; const sales=rows.reduce((s,r)=>s+r._qty,0), target=totalTarget(), ach=target?sales/target*100:0, gap=Math.max(target-sales,0);
  ...
  const byDate=sumBy(rows.map(r=>({...r,_d:r._date.toISOString().slice(0,10)})),'_d'); const dateLabels=byDate.map(x=>new Date(x[0]).getDate()); const daily=byDate.map(x=>x[1]); const pace=target/daysInMonth;
...
function renderModelTable(rows){
  const salesMap=new Map(sumBy(rows,'_model')); const invMap=new Map(sumBy(inventoryRows().map(r=>({...r,_qty:r._inv})),'_model')); const targetMap=new Map(); relevantTargets().forEach(t=>{const m=find(t,'Model','SKU','Product');targetMap.set(m,(targetMap.get(m)||0)+n(find(t,'Target','Qty Target','Sales Target')))});
  const models=uniq([...salesMap.keys(),...targetMap.keys(),...invMap.keys()]); $('modelTableBody').innerHTML=models.map(m=>{const s=salesMap.get(m)||0,t=targetMap.get(m)||0,i=invMap.get(m)||0,a=t?s/t*100:0;return `<tr><td><strong>${escapeHtml(m)}</strong></td><td>${fmt(s)}</td><td>${fmt(t)}</td><td>${pct(a)}</td><td>${fmt(i)}</td></tr>`}).join('')||emptyRow(5);
}
