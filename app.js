...
function productFilterOptions(){const rows=salesEnriched().filter(r=>passes(r._productType,selected('productTypeFilter')));setOptions('modelFilter',uniq(rows.map(r=>r._model)),'models');if($('seriesFilter'))setOptions('seriesFilter',uniq(rows.map(r=>r._series)),'series');}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  for(const selector of ['#productivitySection .productivity-controls','#asmIncentivesSection .asm-controls']){const host=document.querySelector(selector);if(!host)continue;const label=document.createElement('label');label.textContent='Product Type';const select=$('productTypeFilter').cloneNode(true);select.removeAttribute('id');select.setAttribute('aria-label','Product Type');select.dataset.productMirror='true';label.append(select);host.prepend(label);select.addEventListener('change',()=>{$('productTypeFilter').value=select.value;$('productTypeFilter').dispatchEvent(new Event('change',{bubbles:true}));});}
  const syncProductSelectors=()=>document.querySelectorAll('[data-product-mirror]').forEach(s=>s.value=$('productTypeFilter').value);const beforeProductRender=render;render=()=>{syncProductSelectors();beforeProductRender();};
