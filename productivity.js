...
function productivityTotal(groups){
 return groups.reduce((total,g)=>{for(const key of ['headcount','high','safe','improve','low'])total[key]+=g[key];return total;},{label:'WVIS',headcount:0,high:0,safe:0,improve:0,low:0});
}
(()=>{
 ...
 function renderProductivity(){
  ...
  const label={area:'Area',subregion:'Area / Subregion',dealer:'Dealer'}[view];
  const rowHtml=g=>'<tr><td>'+escapeHtml(g.label)+'</td><td>'+g.headcount+'</td>'+['high','safe','improve','low'].map(key=>'<td data-sort-value="'+(report.ready?g[key]:'')+'">'+(report.ready?g[key]+' ('+pct(g.headcount?g[key]/g.headcount*100:0)+')':'—')+'</td>').join('')+'</tr>';
