...
function modelGapCell(current,previous,totalDecline,summary=false,currency=false,score=false){
 ...
 const format=currency?php:score?v=>fmt(v,2):fmt;
 const title=score?'Score gap; percentage is the share of total score decline.':currency?'Net sales amount gap; percentage is the share of total sales amount decline.':summary?'Net unit gap; percentage totals the decline shares of displayed rows. Total lost units: '+fmt(totalDecline):'Share of total decline: lost units divided by '+fmt(totalDecline)+' lost units across displayed rows.';
...
function renderModelHistory(rows,bodyId='modelTableBody',groupKey='_model',entityLabel='Model',options={}){
  ...
  if(isScore){labels[1]+=' Score';labels[2]+=' Score';labels[3]='MTD Score Gap (Decline Share)';}
...
function renderFinancials(){
 ...
 const month=selected('monthFilter');if(!/^\d{4}-\d{2}$/.test(month)){$('lineupTableBody').innerHTML=emptyRow(5);return;}
 const previous=modelMonthOffset(month,-1);
 const filters=[['_area',selected('areaFilter')],['_asm',selected('asmFilter')],['_customer',selected('customerFilter')],['_channel',selected('channelFilter')],['_productType',selected('productTypeFilter')],['_model',selected('modelFilter')],['_series',selected('seriesFilter')],['_priceRange',selected('priceRangeFilter')]];
 const labels=['Model','Series','Latest SRP','Date Reference','Agreed Score / Unit · '+monthName(month)];
 ...
 $('lineupPeriod').textContent=monthName(previous)+' + '+monthName(month)+' · model availability and price reference. All filters apply.';
 $('lineupTableBody').innerHTML=lineup.map(g=>{const prices=[...g.prices].sort((a,b)=>a-b),low=prices[0],high=prices[prices.length-1];const price=prices.length?(low===high?php(low):php(low)+' – '+php(high)):'—',points=scoreCatalog.get(modelKey(g.model))?.points;return '<tr><td><strong>'+escapeHtml(g.model)+'</strong></td><td>'+escapeHtml(g.series)+'</td><td data-sort-value="'+(low??'')+'">'+price+'</td><td>'+(g.latest||'—')+'</td><td data-sort-value="'+(points??'')+'">'+(points==null?'Not set':fmt(points,2))+'</td></tr>';}).join('')||emptyRow(5);
}
...
const financialStyle=document.createElement('style');financialStyle.textContent='.kpi-card[hidden]{display:none}#salesAmountKpi{font-size:clamp(20px,2vw,32px)}#lineupTableBody td{font-variant-numeric:tabular-nums}#lineupTableBody td:nth-child(3){white-space:nowrap}';document.head.appendChild(financialStyle);
...
function entitySeriesReport(all,rows,month,measure='qty'){
 const value=r=>measure==='amount'?(salesAmount(r)??0):measure==='score'?(r._points??0):r._qty;
 ...
 for(const r of rows){const d=day(r);if(d===null||!((d>=start&&d<=cutoff)||(d>=pstart&&d<=pend)||(d>=weeks[4].start&&d<=cutoff)))continue;const name=r._series||'Unmapped series';if(!groups.has(name))groups.set(name,blank(name));const g=groups.get(name);if(d>=start&&d<=cutoff){g.current+=value(r);const amount=salesAmount(r);if(amount===null)g.missing++;else g.amount+=amount;}if(d>=pstart&&d<=pend)g.previous+=value(r);weeks.forEach((w,i)=>{if(d>=w.start&&d<=w.end)g.weeks[i]+=value(r);});}
...
(()=>{
 ...
 function showDetail(b){const all=salesEnriched(),month=selected('monthFilter'),filters=[['_area',selected('areaFilter')],['_asm',selected('asmFilter')],['_customer',selected('customerFilter')],['_channel',selected('channelFilter')],['_productType',selected('productTypeFilter')],['_model',selected('modelFilter')],['_series',selected('seriesFilter')],['_priceRange',selected('priceRangeFilter')]];
  ...
  dialog.innerHTML+='<div class="top50-dealer-controls" role="group" aria-label="Performance measure">'+['qty','amount',...(b.dataset.entityDetail==='psSalesReviewBody'?['score']:[])].map(m=>'<button type="button" class="secondary-btn" data-detail-measure="'+m+'" aria-pressed="'+(m===detailMeasure)+'">'+({qty:'Sales QTY',amount:'Sales Amount (₱)',score:'Scores'}[m])+'</button>').join('')+'</div>';
  if(!report)dialog.innerHTML+='<p>No sales dates available for the selected month.</p>';else{const r=report;
   const row=g=>'<tr><th>'+escapeHtml(g.name)+'</th><td>'+format(g.current)+'</td><td>'+(r.previousAvailable?format(g.previous):'N/A')+'</td><td>'+(r.previousAvailable?format(g.current-g.previous):'N/A')+'</td><td>'+ir(g.current,r.previousAvailable?g.previous:null)+'</td>'+r.weeks.slice(0,4).map((w,i)=>'<td>'+(w.available?format(g.weeks[i]):'N/A')+' ('+ir(w.available?g.weeks[i]:null,r.weeks[i+1].available?g.weeks[i+1]:null)+')</td>').join('')+'</tr>';
   dialog.innerHTML+='<p>'+escapeHtml(monthName(month)+' · '+date(r.start)+'–'+date(r.cutoff))+'</p><div class="entity-detail-kpis"><strong>'+label+': '+format(r.total.current)+'</strong></div><div class="table-wrap"><table data-no-sort><thead><tr><th>Smartphone Series</th><th>'+historyMonthLabel(month)+' MTD</th><th>'+historyMonthLabel(modelMonthOffset(month,-1))+' MTD</th><th>Gap '+label+'</th><th>Change %</th>'+r.weeks.slice(0,4).map(w=>'<th>'+date(w.start)+'–'+date(w.end)+'<br>'+label+' (IR)</th>').join('')+'</tr></thead><tbody>'+r.list.map(row).join('')+'</tbody><tfoot>'+row(r.total)+'</tfoot></table></div><p>Same-period comparison through day '+new Date(r.cutoff*86400000).getUTCDate()+'. Weekly IR compares with the preceding 7 days. N/A indicates missing date coverage or zero previous sales. Series ordered by largest decline in the selected measure.</p>';
  }dialog.querySelector('[data-entity-close]').onclick=()=>dialog.close();if(!dialog.open)dialog.showModal();
 }
...
// Totals describe the current ACTIVE HR roster, including members with no sales.
function effectiveRosterTotals(roster,kind,filters,all,onlyTop50=false){
 if(!roster)return null;
 const norm=v=>String(v||'').trim().replace(/\s+/g,' ').toLowerCase(),latest=new Map(),groups=new Map(),unique=new Set();
 for(const r of all){if(!r._date||isNaN(r._date))continue;for(const key of [r._sid?'store:'+norm(r._sid):'',r._ps?'ps:'+norm(r._ps):''])if(key&&(!latest.has(key)||r._date>=latest.get(key)._date))latest.set(key,r);}
 for(const entry of roster.entries){
  const dealer=entry.customer||'Unassigned',record=latest.get('ps:'+norm(entry.id))||latest.get('store:'+norm(entry.sid)),values={_area:entry.area,_asm:entry.asm,_customer:dealer,_channel:record?._channel||'Unassigned'};
  if(onlyTop50&&!isTop50Dealer(dealer))continue;
  if(!filters.filter(([key])=>['_area','_asm','_customer','_channel'].includes(key)).every(([key,value])=>value==='ALL'||norm(values[key])===norm(value)))continue;
  const key=kind==='effectiveStore'?(entry.sid?'id:'+norm(entry.sid):entry.store?'name:'+norm(entry.store):''):(entry.id?'id:'+norm(entry.id):'name:'+norm(entry.name));
  if(!key)continue;
  const dealerKey=norm(dealer);if(!groups.has(dealerKey))groups.set(dealerKey,{dealer,keys:new Set()});groups.get(dealerKey).keys.add(key);unique.add(key);
 }
 return {groups,total:unique.size};
}
...
function renderEffectiveDealers(){
 ...
 if(rosterTotals){const seen=new Set(report.dealers.map(dealerKey));for(const group of rosterTotals.groups.values())if(!seen.has(dealerKey(group.dealer))){report.dealers.push(group.dealer);seen.add(dealerKey(group.dealer));}report.dealers.sort((a,b)=>a.localeCompare(b));}
 ...
 labels.splice(1,0,dealerSalesMeasure==='effectiveStore'?'Total Stores':'Total Promoters');
 table.tHead.innerHTML='<tr>'+labels.slice(0,6).map((l,i)=>'<th rowspan="2" scope="col" data-sort-column="'+i+'">'+escapeHtml(l)+'</th>').join('')+'<th colspan="3" scope="colgroup">Monthly Effective '+entity+' Count</th><th colspan="3" scope="colgroup">Increase Rate</th></tr><tr>'+labels.slice(6).map((l,i)=>'<th scope="col" data-sort-column="'+(i+6)+'">'+escapeHtml(l)+'</th>').join('')+'</tr>';
 ...
 const totalCell=dealer=>{const total=rosterTotals?(dealer===null?rosterTotals.total:rosterTotals.groups.get(dealerKey(dealer))?.keys.size||0):null;return '<td data-sort-value="'+(total??'')+'" title="Current ACTIVE HR list: unique promoters or their assigned stores. Territory/customer/channel filters apply; product filters do not reduce the roster total. Historical effective counts may include resigned promoters.">'+(total===null?'—':fmt(total))+'</td>';};
 const row=dealer=>{const current=count(report.periods[0],dealer),previous=count(report.periods[1],dealer);return '<tr><'+(dealer===null?'th scope="row"':'td')+'>'+(dealer===null?'WVIS':'<button type="button" class="effective-detail-button"'+attrs(dealer,0)+'>'+escapeHtml(dealer)+'</button>')+'</'+(dealer===null?'th':'td')+'>'+totalCell(dealer)+cell(dealer,0)+cell(dealer,1)+gap(current,previous,dealer===null)+modelRateCell(current,previous)+[3,4,5].map(i=>cell(dealer,i)).join('')+[3,4,5].map(i=>modelRateCell(count(report.periods[i],dealer),count(report.periods[i-1],dealer))).join('')+'</tr>';};
 $('dealerTableBody').innerHTML=report.dealers.map(row).join('')||emptyRow(12);
