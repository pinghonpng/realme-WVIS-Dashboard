...
function renderFinancials(){
 ...
 $('lineupTableBody').innerHTML=lineup.map(g=>{const prices=[...g.prices].sort((a,b)=>a-b),low=prices[0],high=prices[prices.length-1];const price=prices.length?(low===high?php(low):php(low)+' – '+php(high)):'—';return '<tr><td><strong>'+escapeHtml(g.model)+'</strong></td><td>'+escapeHtml(g.series)+'</td><td>'+format(g.previous)+'</td><td>'+format(g.current)+'</td><td>'+fmt(g.previous+g.current)+'</td><td data-sort-value="'+(low??'')+'">'+price+'</td><td>'+ (g.latest||'—')+'</td></tr>';}).join('')||emptyRow(7);
}
